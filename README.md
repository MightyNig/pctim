...
- `Custody` = the active custody condition is valid;
- `State` = the payload state matches the active state condition at the verification boundary;
...
Each carrier execution produces a boundary receipt containing the test outcome. A cryptographic boundary commitment (receipt_hash) is generated by the carrier core over the full receipt payload. The independent verifier recomputes the commitment and compares it with the stored value. 
...
- the published receipt artifact cannot be independently verified from its stored commitments;
...
The carrier guarantees deterministic behavioral outcomes under the published test conditions. In particular, an invalid condition produces NO_BIND and does not advance the protected-effect counter. Because the temporal decay test evaluates live wall-clock nanoseconds,the measured elapsed time in that receipt may differ across independent runs. Therefore, the stale-continuation receipt bytes and its derived cryptographic hashes may differ between runs.

The public claim is strict behavioral reproducibility, not byte-identical cryptographic replay across independent runs.
